...
class Camera:
    ...
    def __init__(self, position: Point, look_dir: Vector,
        ...
        """
        Перемещение камеры вперёд, назад, в стороны, вверх. вниз,
        по направлению камеры (возможно).
        Коэффициент для перемещения камеры по диагонали = 1/2.

        """
    
    def send_rays(self) -> list[list[Ray]]:
        """
        Сравнение длин лучей
        Проецирование векторов на касательную (прямую или плоскость)
        к сфере.
        Нормализация вектора, относительно вектора направления камеры
        v'_k = v_k / cos \gamma,
        где cos \gamms = cos(угол между v и v_k)
...
class Events:
    ...
    def trigger(cls, ev, *args):
        ...
        """
        """
        Функции, которые исполняются в соответствии с стеком событий.
